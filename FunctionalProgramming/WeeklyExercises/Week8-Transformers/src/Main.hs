...
mgu (TFun l r) (TFun l' r')
   =  do  m_s1 <- mgu l l'
          case m_s1 of
            Nothing -> throwError "error 1"
            Just s1 -> do
              m_s2 <- mgu (apply s1 r) (apply s1 r')
              case m_s2 of
                Nothing -> throwError "error 2"
                Just s2 -> return (s1 `composeSubst` s2)
...
--ti :: TypeEnv -> Exp -> ExceptT String TI Type
...
ti env (App e1 e2) =
    do  tv <- newTyVar "a"
        ...
        m_s3 <- mgu (apply s2 t1) (TFun t2 tv)
        case m_s3 of
          Nothing -> throwError "error 4"
          Just s3 -> return (s3 `composeSubst` s2 `composeSubst` s1, apply s3 tv)

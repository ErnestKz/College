...
mgu (TFun l r) (TFun l' r')  
  =  mgu l l' >>= \res -> case res of
  (Left x)  -> return $ Left x
  (Right s1) -> (mgu (apply s1 r) (apply s1 r') >>= \res2 -> return (case res2 of
                                                               (Left y)  -> (Left y)
...
mgu (TVar u) t               =  varBind u t >>= \x -> return $ Right $ x
mgu t (TVar u)               =  varBind u t >>= \x -> return $ Right $ x 
...
ti env (App e1 e2) =
    do  tv <- newTyVar "a"
        ...
        return (case (s1, s2, s3) of
                  ((Right x1), (Right x2), (Right x3)) -> (x3 `composeSubst` x2 `composeSubst` x1, apply x3 tv)
                    _ -> (Left "Error"))

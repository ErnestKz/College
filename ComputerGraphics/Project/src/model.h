...
public:
  ...
  {
    loadModel(path);
    hierarchy = MeshHierarchy(&meshes);
  }

  Model(string const &path, string const &model_hierarchy_path, bool gamma = false) : gammaCorrection(gamma)
  {
    loadModel(path);
    hierarchy = MeshHierarchy(&meshes, model_hierarchy_path);
  }
